...
class Settings(BaseModel):
    ...
    vector_db_api_key: str =  os.getenv('PINECONE_API_KEY')
    vector_db_url: Optional[str] =  os.getenv('QDRANT_URL')
    vector_db_index_name: Optional[str] =  os.getenv('PINECONE_INDEX_NAME')
    vector_db_collection: Optional[str] =  os.getenv('QDRANT_COLLECTION_NAME')
    ...
    vector_db_persist_directory: Optional[str] =  os.getenv('VECTOR_DB_PERSIST_DIRECTORY')
    
    # Embedding Settings
    embedding_provider: str = os.getenv('EMBEDDING_TYPE')
    embedding_model: str= os.getenv('VOYAGE_EMBED_MODEL')
    
    # LLM Settings
    llm_provider: str = os.getenv('LLM_TYPE')
    llm_model: str = os.getenv('LLM_MODEL') # Default value provided
